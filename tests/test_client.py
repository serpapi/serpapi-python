...
class TestClient:
    ...
    async def test_html_success(self):
        ...
        with patch.object(client, '_make_request', return_value=mock_html) as mock_request:
    ...
    async def test_location_success(self):
        ...
        mock_locations = [
            {"id": "1", "name": "Austin, TX", "country_code": "US"}
        ]
        
        with patch.object(client, '_make_request', return_value=mock_locations) as mock_request:
            result = await client.location({"q": "Austin", "limit": 3})
            
            mock_request.assert_called_once_with('/locations.json', {"q": "Austin", "limit": 3}, 'json')
    ...
    async def test_search_archive_success(self):
        ...
        with patch.object(client, '_make_request', return_value=mock_archive) as mock_request:
    ...
    async def test_account_success(self):
        ...
        with patch.object(client, '_make_request', return_value=mock_account) as mock_request:
    ...
    async def test_account_with_api_key(self):
        ...
        with patch.object(client, '_make_request', return_value=mock_account) as mock_request:
            ...
            mock_request.assert_called_once_with('/account', {"api_key": "custom_key"}, 'json')
            assert result == mock_account
