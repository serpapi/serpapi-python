...
class TestSerpApi(unittest.TestCase):
    ...
    def test_search(self):
        client = serpapi.Client({"engine": "google", "api_key": os.getenv("API_KEY")})
        data = client.search({"q": "Coffee", "location": "Austin,Texas"})
    ...
    def test_invalid_api_key(self):
        client = serpapi.Client({"engine": "google", "api_key": "invalid_api_key"})
    ...
    def test_error_missing_engine(self):
        client = serpapi.Client({"api_key": os.getenv("API_KEY"), "engine": ""})
        with pytest.raises(
            serpapi.SerpApiException, match=r"Unsupported.*search engine."
        ):
